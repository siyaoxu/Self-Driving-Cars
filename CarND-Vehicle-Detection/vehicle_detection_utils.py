...
def load_data(path,img_type = 'png'):
    '''
    load images given a folder
    '''
...
#    print(list(os.walk(path)))
    for root, dirnames, filenames in os.walk(path):
        for filename in fnmatch.filter(filenames, '*.'+img_type):
            img_list.append(cv2.imread(os.path.join(root, filename)))
    return img_list
...
def extract_features(imgs, cspace='BGR', spatial_size=(32, 32),
                     ...
                     orient=9, pix_per_cell=8, cell_per_block=2, vis=False, feature_vec=True, hog_channel='ALL'):
    '''
    ...
    for image in imgs:
        ...
        # hog_features = get_hog_features(BGR2GRAY(feature_image), orient, pix_per_cell, cell_per_block, vis, feature_vec)
        # Append the new feature vector to the features list
        features.append(np.concatenate((spatial_features, hist_features, hog_features)))
    # Return list of feature vectors
    return features

def createX(featureList, normalize = False):
    '''
    create X array for scikit-learn classifier
    '''
...
def createY(car_features, notcar_features):
    '''
    create a labels vector based on features lists
    '''
    return np.hstack((np.ones(len(car_features)), \
...
def draw_boxes(img, bboxes, color=(0, 0, 255), thick=6):
    '''
    draw sliding windows on the image
    '''
...
def search_windows(img, windows, clf, scaler, color_space='RGB',
    ...
    '''
    detect whether pixels in searched windows are cars
    '''
